...
/// `shift`，没有 `alt`——而本项目的默认热键正是 Ctrl+Alt+X，捕获式界面根本认不出它。
...
fn brand() -> Element {
    Element::row()
        ...
        .child(
            // 版本号。取 `CARGO_PKG_VERSION`——与关于页、exe 文件属性（`build.rs`）、
            // 发布包名（`scripts/release.ps1`）同一个来源，理由见 `about_rows`。
            //
            // 此处**曾是**「英汉 · 汉英」（查询方向的能力描述）。换掉是因为那句话对
            // 老用户是常驻的废话，而版本号是标题栏上唯一有人真的会去看一眼的东西——
            // 报 bug、对照更新说明、以及分辨手上开着的是不是刚部署的那一份。
            //
...
fn left_pane(st: Rc<State>) -> Element {
    ...
    Element::col()
        ...
        // Ctrl+Alt+X（默认值见 `settings::HotkeySpec`），而窗口内并没有 Ctrl+K 这个
        // 键位。画一组按了没用的
        // 键帽比不画更糟。
