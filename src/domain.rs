...
// ── 富文本 ────────────────────────────────────────────────
//
// 用户词典（MDX）的正文没有字段，只有**带样式的段落**。这套模型放在领域层而不是
...
/// 用户词典的词条：词头 + 一段富文本，没有任何字段化信息。
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct UserEntry {
    pub headword: Headword,
    /// 出处词典的名字。
    ///
    /// **必须显示给用户**，且这不是装饰。随程序分发的两个词库是我们挑过的，用户词典
